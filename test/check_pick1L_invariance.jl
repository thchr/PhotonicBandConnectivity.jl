...
@testset "Invariance to 1L choice" begin
    ...
    data′   = minimal_expansion_of_zero_freq_bands.(sgnums, timereversal=true, verbose=false, shuffle_1Lpick=true);
    ...
    # associated transverse symmetry vectors nᵀ = n - nᴸ
    nsᵀs  = [unique!(sort(transverse_expansion.(Ref(sb), cⁱs, idx¹ᴸ))) for (sb, cⁱs, idx¹ᴸ) in zip(sbs,  cⁱss,  idx¹ᴸs )]
    nsᵀs′ = [unique!(sort(transverse_expansion.(Ref(sb), cⁱs, idx¹ᴸ))) for (sb, cⁱs, idx¹ᴸ) in zip(sbs′, cⁱss′, idx¹ᴸs′)]

    # test that we get the same symmetry vectors for T-branches, regardless of our nᴸ pick
    for (nsᵀ, nsᵀ′) in zip(nsᵀs, nsᵀs′)
        @test nsᵀ == nsᵀ′
    end
end